...
- [ ] (P1) `make check` in this repo cannot reach pytest: `pyright` fails on `main` with 14 pre-existing `reportAttributeAccessIssue` errors (13 in `tests/test_evidence_pack_comment.py`, 1 in `tests/test_cost_emit.py` — all `_RecordingClient`/`CliRunner` result attributes on an inferred `object`), and make aborts the recipe there, so the `pytest` line never runs. VERIFIED 2026-07-28 by a differential: an unmodified clean tree produces the identical 14 errors and the identical no-test outcome. Consequence: the repo's only declared gate is dead, every contributor must know to run `.venv/bin/pytest` by hand, and a test regression cannot be caught by the documented command. This is the inverse of the orchestrator's "`make check` exit 0 does not prove tests ran" invariant — here it never exits 0 at all. Fix: type the fixtures/results properly (not `# type: ignore`), then keep the gate green. Blocks the "Add CI to factory-runner" item above from being meaningful. — added 2026-07-28
- [ ] (P1) The reusable-workflow install pin is guarded only by SHA-identity, not by functional consistency. `tests/test_workflow_contract.py` asserts the workflow contains a literal pinned SHA in its install + `verify-install-revision` steps, so a workflow change that adds a CLI option to an invocation without advancing the pin passes the test and breaks every dispatch at runtime. Exactly this happened: `14907a3` (2026-07-24) added `--execution-file` to the `finalize-run`/`fail-run` invocations and CLI but left the pin at `5ac7981`, whose CLI predates the option — the first real dispatch since (GAP-4, 2026-07-29) failed at `finalize-run` and `fail-run` with `No such option: --execution-file` and stranded the unit. Fix: replace the identity assertion with a check that installs the pinned CLI and verifies it accepts the exact options the workflow passes to `prepare-run`/`classify`/`finalize-run`/`fail-run`/`verify-install-revision` (e.g. `--help` introspection or a dry invocation), and run it in the CI from the "Add CI" item above. Minimal pin-advance already applied (pin → `c266769`); this item is the durable guard. See docs/decisions/0001-reusable-workflow-install-pin-consistency.md — added 2026-07-29
...
- [ ] (P3) The workflow/CLI contract guard is one-directional: it vets .github/workflows/factory-runner.yml against THIS CHECKOUT's CLI, not the CLI at the SHA the workflow actually installs. It catches HEAD drifting away from what the workflow invokes — the direction that caused GAP-4 attempt 1 — but not a pinned revision that predates an option HEAD has. Closing the reverse direction requires installing and introspecting the pinned revision in CI (uv tool install git+...@<pin>, then introspect). Boundary named by the WS-P2.20 build session rather than papered over; recorded in factory-runner CLAUDE.md — added 2026-07-31
